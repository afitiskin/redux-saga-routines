...
describe('bindRoutineToReduxForm', () => {
  ...
  it('should wrap routine into redux-form compatible function', () => {
    ...
    expect(action.meta).to.have.property('noSuccessPayload');
    expect(action.meta.noSuccessPayload).to.equal(false);
  });

  it('should wrap routine into redux-form compatible function with no success payload for old redux-form versions', () => {
    const routine = () => ({ something: true });
    const dispatch = sinon.spy();
    const values = {
      some: 'data',
    ...
    const handler = bindRoutineToReduxForm(routine, true);
    ...
    expect(action.meta).to.have.property('noSuccessPayload');
    expect(action.meta.noSuccessPayload).to.equal(true);
  });
});
