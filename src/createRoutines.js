/**
 * This module is a copy of `createActions` module from 'redux-actions'
 * @see https://github.com/redux-utilities/redux-actions/blob/master/src/createActions.js
 *
 * One difference - it uses `createRoutine` instead of `createAction`
 */
...
function actionMapToActionCreators(actionMap, { prefix, namespace = defaultNamespace } = {}) {
  ...
  return arrayToObject(Object.keys(actionMap), (partialActionCreators, type) => {
    ...
    const actionCreator = isArray(actionMapValue)
      ? createRoutine(prefixedType, ...actionMapValue)
      : createRoutine(prefixedType, actionMapValue);
